...
export function WishlistClient() {
  ...
  return (
    <section className="py-16 md:py-24" aria-label="Wishlist">
      <div className="mx-auto max-w-[1600px] px-4 md:px-8">
        ...
        {saved.length === 0 ? (
          <div className="flex flex-col items-start gap-6 border-t border-bone/10 py-20">
          ...
          <>
            <div className="grid grid-cols-1 gap-x-6 gap-y-14 border-t border-bone/10 pt-14 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {saved.map((product, i) => (
                <ProductCard key={product.slug} product={product} index={i} />
              ))}
            </div>

            <p className="mt-16 text-sm text-smoke">
